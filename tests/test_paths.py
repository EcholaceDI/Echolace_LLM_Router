...
def test_models_dir_respects_env(
...
def test_models_dir_handles_windows_style_paths(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_gguf_dir_defaults_to_models(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_gguf_target_path_uses_gguf_dir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("ECHOLACE_MODELS_DIR", str(tmp_path))
    monkeypatch.delenv("ECHOLACE_GGUF_DIR", raising=False)
...
def test_hf_model_dir_appends_repo(
